...
# Створення об'єкта BeautifulSoup для аналізу HTML-коду головної сторінки
books_soup = BeautifulSoup(response.text, 'html.parser')

# Функція для видалення символів з рядка
...
# Словник для конвертації рейтингу в числовий формат
w_to_d = {
    'One': 1,
    'Two': 2,
    'Three': 3,
...
# Список для зберігання даних про книги
all_books = []
...
# Пошук та обробка даних про книги на кожній сторінці
for page_number in range(1, count):
    ...
    for index, article in enumerate(all_articles, start=1):
        ...
        # Отримання додаткових даних зі сторінки кожної книги
        ...
        # Вивід повідомлення про прогрес
